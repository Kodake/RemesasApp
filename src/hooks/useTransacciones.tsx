...
const useTransacciones = () => {
  ...
  monedaDTO = monedaStore.select.find(
    (moneda) => moneda.idMoneda === Number(store.transaccion.moneda)
  )
  ...
  const handleSaveTransaccion = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        useNotifications(
          'Enviado',
          'Su transacción ha sido realizada satisfactoriamente',
          'success'
        )
      }
    })
  ...
  const handleUpdateTransaccion = async (
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        useNotifications(
          'Retirado',
          'El retiro ha sido realizado satisfactoriamente',
          'success'
        )
      }
    })
  ...
  const handleClipboard = (codigo: string) => {
    navigator.clipboard
      ...
      .then(() => {
        useNotifications(
          'Código copiado',
          'Código de envío copiado en el portapapeles',
          'success'
        )
      })
      .catch((error) => {
  ...
  const handleRequestExchange = async (transaccion: GetTransaccionDTO) => {
    try {
      ...
      if (resultado.isConfirmed) {
        const calculo = calculateExchange(
  ...
  const showMonedaSelectionDialog = async (tasas: GetTasaDTO[]) => {
    const inputOptions = tasas.reduce(
      (options, tasa) => {
        options[tasa.idTasa] =
          `${tasa.monedaOrigen.split('-')[1]} a ${tasa.monedaDestino.split('-')[1]} = ${tasa.valor}`
        return options
      },
    ...
    return await Swal.fire({
      ...
      preConfirm: async (moneda) => {
        const tasaDTO = tasaStore.select.find(
          (tasa) => tasa.idTasa === Number(moneda)
        )
        return tasaDTO
      },
  ...
  const findMonedaIdByCodigo = (codigo: string) => {
    const moneda = monedaStore.select.find((moneda) =>
      moneda.codigo.includes(codigo.split('-')[1].trim())
    )
    return moneda?.idMoneda
  }

  const confirmExchange = async (
    transaccion: GetTransaccionDTO,
    selectedTasa: any,
    calculo: number,
    ...
    if (confirmationResult.isConfirmed) {
      ...
      useNotifications(
        'Intercambio',
        'El cambio de remesa ha sido realizado satisfactoriamente',
        'success'
      )
    }
  }
